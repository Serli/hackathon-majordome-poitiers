    ...
    async.waterfall([
        function(callback) {
            ...
            if (pub.getOption("demosEnable")) {
                ...
                pub.httpApp.get(pub.getOption("clientPublicFolder") + "/*", function(req, res) {
                    (res.sendFile||res.sendfile).call(res,
                        "./client-webrtc/" + (req.params[0] ? req.params[0] : "index.html"),
                        // TODO security check
                        {root:__dirname + "/../"},
                        function(err) {
                            try{if (err && err.status && res && !res._headerSent) {
                ...
                pub.httpApp.get(pub.getOption("clientPublicFolder"), function(req, res) {res.redirect(pub.getOption("clientPublicFolder") + "/");});
            }
